...
//Serial* SP = new Serial(port_name);
//int value;
//DWORD bytesSend;
	...
	case NSK_ALGO_CB_TYPE_ALGO:
	{
		...
		case NSK_ALGO_TYPE_MED:
		{
			...
			if (med > 90) {
				...
				write();
				iter = z;
				swprintf(buffer, 512, L"MED DETECTED", med);
				OutputToEditBox(buffer);
				OutputLog(buffer);
				break;
			}
		...
		case NSK_ALGO_TYPE_BLINK:
		{
			SYSTEMTIME x;
			GetSystemTime(&x);
			if ((x.wMinute * 60 * 1000 + x.wSecond * 1000 + x.wMilliseconds) - (y.wMinute * 60 * 1000 + y.wSecond * 1000 + y.wMilliseconds) > 700) {
				int strength = param.param.index.value.group.eye_blink_strength;
				//if (prevs != 0)
				{ //Blink
					swprintf(buffer, 512, L"Eye Blink Detected");
					OutputToEditBox(buffer);
					OutputLog(buffer);
					char str[2];
					str[0] = iter;
					str[1] = '\n';
					//str[2] = '\n';
					swprintf(buffer, 512, L"sending");
					OutputToEditBox(buffer);
					OutputLog(buffer);
					write();
					swprintf(buffer, 512, L"sent value : %d", str[0]);
					OutputToEditBox(buffer);
					OutputLog(buffer);
				}
				//prevs = strength;
			...
			{ //Double Blink
				...
				swprintf(buffer, 512, L"Double Eye Blink Detected");
				OutputToEditBox(buffer);
				OutputLog(buffer);
				//prevs = 0;
				y = x;
...
static void UIComponentsPositionUpdate(HWND hWnd) {
	...
	SetWindowPos(hMedCheck, NULL, rect.right - BUTTON_WIDTH - MARGIN, MARGIN + BUTTON_HEIGHT + MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT, SWP_NOZORDER);
	...
	SetWindowPos(hOutputText, NULL, MARGIN, MARGIN + (BUTTON_HEIGHT + MARGIN) * 4 + MARGIN, rect.right - (MARGIN * 2)- SIGNAL_TEXT_WIDTH, rect.bottom - (MARGIN + (BUTTON_HEIGHT + MARGIN) * 4 + MARGIN) - BUTTON_HEIGHT - (MARGIN * 2), SWP_NOZORDER);
	//SetWindowText(hOutputText, L"1\r\n2\r\n3\r\n");
	SetWindowPos(hAngles, NULL,MARGIN + MARGIN + rect.right - (MARGIN * 2) - SIGNAL_TEXT_WIDTH, MARGIN + (BUTTON_HEIGHT + MARGIN) * 4 + MARGIN, SIGNAL_TEXT_WIDTH - MARGIN, rect.bottom - (MARGIN + (BUTTON_HEIGHT + MARGIN) * 4 + MARGIN) - BUTTON_HEIGHT - (MARGIN * 2), SWP_NOZORDER);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		/* hWnd1 = CreateWindowEx(0, szWindowClass, L"Child window", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, 320, 480, hWnd, NULL, hInst, NULL);
		...
		hOutputText = CreateWindow(L"Edit", L"", WS_BORDER | WS_VISIBLE | WS_CHILD | WS_VSCROLL | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL, 0, 0, 10, 10, hWnd, (HMENU)IDD_OUTPUT_TEXT, hInst, NULL);

		hAngles = CreateWindow(L"Static", L"          INSTRUCTIONS:\n\n   1 TO MOVE ARM 1CM IN\n\t+VE X-AXIS\n\n   2 TO MOVE ARM 1CM IN\n\t+VE Y-AXIS\n\n   3 TO MOVE ARM 1CM IN\n\t+VE Z-AXIS\n\n   4 TO MOVE ARM 1CM IN\n\t-VE X-AXIS\n\n   5 TO MOVE ARM 1CM IN\n\t-VE Y-AXIS\n\n   6 TO MOVE ARM 1CM IN\n\t-VE Z-AXIS\n      MED TO OPERATE\n\tGRIPPER", WS_BORDER|WS_VISIBLE | WS_CHILD , 0, 0, 100, 40, hWnd, (HMENU)IDD_OUTPUT_TEXT, hInst, NULL);
		...
		hBlinkCheck = CreateWindow(L"Button", L"Blink", WS_VISIBLE | WS_CHILD | BS_CHECKBOX|WS_BORDER, 10, 10, 2*BUTTON_WIDTH, 2*BUTTON_HEIGHT, hWnd, (HMENU)IDD_AP_CHECK, hInst, NULL);
		...
		hMedCheck = CreateWindow(L"Button", L"Med", WS_VISIBLE | WS_CHILD | BS_CHECKBOX|WS_BORDER, 10, 10, BUTTON_WIDTH, BUTTON_HEIGHT, hWnd, (HMENU)IDD_MED_CHECK, hInst, NULL);
